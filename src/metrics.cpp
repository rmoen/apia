...
int printStats(bool mining)
{
    ...
    if (IsInitialBlockDownload()) {
        ...
        std::cout << "     " << _("Downloading blocks") << " | " << BLUE << height << " / ~" << netheight << " (" << downloadPercent << "%)" << WHITE << std::endl;
    } else {
        std::cout << "           " << _("Block height") << " | " << BLUE << height << WHITE << std::endl;
    }
    std::cout << "            " << _("Connections") << " | " << RED << connections << WHITE << std::endl;
    std::cout << "  " << _("Network solution rate") << " | " << YELLOW << netsolps << " Sol/s" << WHITE << std::endl;
    if (mining && miningTimer.running()) {
        std::cout << "    " << _("Local solution rate") << " | " << GREEN << strprintf("%.4f Sol/s", localsolps) << WHITE << std::endl;
        lines++;
    }
    std::cout << std::endl;
...
int printMiningStatus(bool mining)
{
    ...
    if (mining) {
        ...
        std::cout << _("To enable mining, add 'gen=1' to your apia.conf and restart.") << std::endl;
        lines += 2;
    }
    std::cout << std::endl;
            ...
            while (it != u->end()) {
                ...
                if (mapBlockIndex.count(hash) > 0 &&
                    ...
                    if ((height > 0) && (height <= consensusParams.GetLastFoundersRewardBlockHeight())) {
                        subsidy -= subsidy/16.6666666666666666;
                    }
